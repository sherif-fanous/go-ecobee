...
func (c *Client) Group(ctx context.Context, selection *objects.Selection) (*GroupSuccessResponse, error) {
	...
	resp, err := c.get(ctx, fmt.Sprintf("%s%d/%s", apiBaseURL, apiVersion, groupEndpoint), queryParameters, map[string][]string{"Authorization": {fmt.Sprintf("%s %s", c.tokenType, c.accessToken)}})
	if err != nil {
		return nil, fmt.Errorf("%s: %w", groupEndpoint, err)
	}
...
func (c *Client) UpdateGroup(ctx context.Context, selection *objects.Selection, groups []objects.Group) (*GroupSuccessResponse, error) {
	...
	resp, err := c.post(ctx, fmt.Sprintf("%s%d/%s", apiBaseURL, apiVersion, groupEndpoint), queryParameters, map[string][]string{"Authorization": {fmt.Sprintf("%s %s", c.tokenType, c.accessToken)}}, bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("%s: %w", groupEndpoint, err)
	}
