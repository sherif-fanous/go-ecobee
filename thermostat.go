...
func (c *Client) Thermostat(ctx context.Context, selection *objects.Selection, page *objects.Page) (*ThermostatSuccessResponse, error) {
	...
	resp, err := c.get(ctx, fmt.Sprintf("%s%d/%s", apiBaseURL, apiVersion, thermostatEndpoint), queryParameters, map[string][]string{"Authorization": {fmt.Sprintf("%s %s", c.tokenType, c.accessToken)}})
	if err != nil {
		return nil, fmt.Errorf("%s: %w", thermostatEndpoint, err)
	}
...
func (c *Client) ThermostatSummary(ctx context.Context, selection *objects.Selection) (*ThermostatSummarySuccessResponse, error) {
	...
	resp, err := c.get(ctx, fmt.Sprintf("%s%d/%s", apiBaseURL, apiVersion, thermostatSummaryEndpoint), queryParameters, map[string][]string{"Authorization": {fmt.Sprintf("%s %s", c.tokenType, c.accessToken)}})
	if err != nil {
		return nil, fmt.Errorf("%s: %w", thermostatSummaryEndpoint, err)
	}
...
func (c *Client) UpdateThermostat(ctx context.Context, selection *objects.Selection, thermostat *objects.Thermostat, functions []objects.Function) (*APIStatusResponse, error) {
	...
	resp, err := c.post(ctx, fmt.Sprintf("%s%d/%s", apiBaseURL, apiVersion, thermostatEndpoint), queryParameters, map[string][]string{"Authorization": {fmt.Sprintf("%s %s", c.tokenType, c.accessToken)}}, bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("%s: %w", thermostatEndpoint, err)
	}
